...
public class DualAntennaHandler implements INxpNfcNtfHandler {
    ...
    public static final Object sDualAntennaSync = new Object();
    private static int sAntennaSelected = 0x00;
    private final NfcOperations mNfcOperations;
    private static final int APPEND_Q_POLL = 0X02;
    private static final int ONLY_Q_POLL = 0X01;
    private static final int Q_TAG_MODE_INDEX = 1;
    ...
    private static int sAntennaPolling = 0x00;
    ...
    public enum DualAntennaStatusCode {
        ...
        DualAntennaStatusCode(int value) {
    ...
    public boolean isDualAnetannaSupported() throws IOException {
        ...
        byte[] payload = {(byte) (DUAL_ANTENNA_SUB_GID_OID | DualAntennaSubOid.isSupported.value)};
        try {
            byte[] isSupported =
    ...
    public void enableQTagDualAntenna(int mode) throws IOException {
        ...
        byte[] qtag = {QTAG_SUB_GID_OID, (byte) QTagMode.DISABLE_QTAG_MODE.getValue()};
    ...
    public @DualAntennaStatus int setDiscoveryTechnology_DualAntenna(
            int antennaOneTech, int antennaTwoTech) throws IOException {
        ...
        try {
            ...
            Set<Integer> validValues =
                    Set.of(
                            RfPollingStatus.passive_ABF.value,
            ...
            synchronized (sDualAntennaSync) {
                sAntennaSelected = 0x00;
            }
            byte[] payload = {
                (byte) (DUAL_ANTENNA_SUB_GID_OID | DualAntennaSubOid.setDiscoveryTechnology.value),
                (byte) antennaOneTech,
                (byte) antennaTwoTech
            };

            byte[] setDiscoveryTechnology =
                    mNxpNciPacketHandler.sendVendorNciMessage(
                            NxpNfcConstants.NFC_NCI_PROP_GID,
                            NxpNfcConstants.NXP_NFC_PROP_OID,
                            payload);
            if (setDiscoveryTechnology != null
                    && setDiscoveryTechnology.length > 0
                    && setDiscoveryTechnology[1] == NfcAdapter.SEND_VENDOR_NCI_STATUS_SUCCESS) {
    ...
    public @DualAntennaStatus int setDiscoveryTechnology_DualAntenna(
            int antennaOneTech, int antennaTwoTech, DualAntennaCallback mDualAntennaCallback)
            throws IOException {
        int pollTech;
        int status = DualAntennaStatusCode.Failed.value;
        if (mDualAntennaCallback == null) return DualAntennaStatusCode.Failed.value;
        try {
    ...
    public @DualAntennaStatus int setPollingMode_DualAntenna(
        ...
        } else if (ant1 == ReaderMode.Enable.value && ant2 == ReaderMode.Disable.value) {
            readerMode = ConDiscParam.EnableAnt1.value;
        } else if (ant1 == ReaderMode.Disable.value && ant2 == ReaderMode.Enable.value) {
            readerMode = ConDiscParam.EnableAnt2.value;
        } else {
            NxpNfcLogger.d(TAG, "only card mode enabled");
        }
        byte[] payload = {
            (byte) (DUAL_ANTENNA_SUB_GID_OID | DualAntennaSubOid.enableReaderMode.value),
            (byte) readerMode
        };
        try {
            byte[] enableReaderMode =
                    mNxpNciPacketHandler.sendVendorNciMessage(
                            NxpNfcConstants.NFC_NCI_PROP_GID,
                            NxpNfcConstants.NXP_NFC_PROP_OID,
                            payload);
            if (enableReaderMode != null
                    && enableReaderMode.length > 0
                    && enableReaderMode[1] == NfcAdapter.SEND_VENDOR_NCI_STATUS_SUCCESS) {
                sAntennaPolling = readerMode;
                return DualAntennaStatusCode.Success.value;
            } else {
                return DualAntennaStatusCode.Failed.value;
            }
    ...
    public int[] getDiscoveryTechnology_DualAntenna() throws IOException {
        NxpNfcLogger.d(TAG, "getDiscoveryTechnology_DualAntenna framework Enter");
        byte[] payload = {
            (byte) (DUAL_ANTENNA_SUB_GID_OID | DualAntennaSubOid.getDiscoveryTechnology.value)
        };
        try {
            int[] rsp = new int[2];
            byte[] getDiscoveryTechnology =
                    mNxpNciPacketHandler.sendVendorNciMessage(
                            NxpNfcConstants.NFC_NCI_PROP_GID,
                            NxpNfcConstants.NXP_NFC_PROP_OID,
                            payload);
