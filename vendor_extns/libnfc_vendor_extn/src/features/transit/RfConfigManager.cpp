...
std::unique_ptr<RfConfigManager> RfConfigManager::instance = nullptr;
...
RfConfigManager *RfConfigManager::getInstance() {
  if (!instance) {
    instance = std::unique_ptr<RfConfigManager>(new RfConfigManager());
  }
  return instance.get();
}
...
bool RfConfigManager::isRFConfigUpdateRequired(
  ...
  do {
    ...
    propCmdResData[index] = static_cast<uint8_t>(newValue & BYTE0_SHIFT_MASK);
...
bool RfConfigManager::isUpdateThreshold(uint8_t index, unsigned &new_value,
  ...
  if (read_value != new_value) {
    lpdet_cmd_response[index] = static_cast<uint8_t>(new_value & BYTE0_SHIFT_MASK);
    lpdet_cmd_response[index + 1] =
        static_cast<uint8_t>((new_value & BYTE1_SHIFT_MASK) >> 8);
...
bool RfConfigManager::checkUpdateRfRegisterConfig(
  ...
  rf_reg_A085_value = static_cast<unsigned>((cmd_response[REG_A085_DATA_INDEX + 3] << 24) |
  ...
  while (getline(key_value_pairs, single_key_value)) {
    ...
    if (!ParseUint(value.c_str(), &new_value)) {
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "%s: Failed to parse value: %s",
    ...
    case UPDATE_CN_TRANSIT_BLK_NUM_CHECK_ENABLE: {
      ...
      if (itReg == rfRegA085Map.end())
        continue;
      b_position = static_cast<unsigned>(0x01 << itReg->second);
    ...
    if (index_to_value) {
      ...
      mRspBuffer[index_to_value] = static_cast<uint8_t>(new_value & BYTE0_SHIFT_MASK);
      if (update_mode == BYTEWISE) {
  ...
  if (is_feature_update_required) {
    ...
    cmd_response[REG_A085_DATA_INDEX + 3] =
        static_cast<uint8_t>((rf_reg_A085_value & BYTE3_SHIFT_MASK) >> 24);
    cmd_response[REG_A085_DATA_INDEX + 2] =
        static_cast<uint8_t>((rf_reg_A085_value & BYTE2_SHIFT_MASK) >> 16);
    cmd_response[REG_A085_DATA_INDEX + 1] =
        static_cast<uint8_t>((rf_reg_A085_value & BYTE1_SHIFT_MASK) >> 8);
    cmd_response[REG_A085_DATA_INDEX] =
        static_cast<uint8_t>(rf_reg_A085_value & BYTE0_SHIFT_MASK);
