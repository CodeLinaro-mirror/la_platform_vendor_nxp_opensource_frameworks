...
bool NxpNTag::isNtagSupported() {
  ...
  if (mFwVer < DEFAULT_NTAG_SUPPORT_MIN_FW_VER) {
    NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                   "%s:FW not supported for Ntag detection feature", __func__);
    return false;
  }
...
void NxpNTag::phNxpNciHal_disableNtagNtfConfig() {
  ...
  if (mFwVer >= DEFAULT_LPCD_WITHOUT_POLL_SUPPORT_MIN_FW_VER) {
    ...
    status = PlatformAbstractionLayer::getInstance()->palNfcSendExtCmd(
        sizeof(LPCD_WOUT_POLL_SET_OFF), LPCD_WOUT_POLL_SET_OFF, &rsp_len, rsp);
    if (status != NFCSTATUS_SUCCESS) {
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                     "%s:LPCD_WOUT_POLL_SET_OFF: Failed", __func__);
    }
  }
  ...
  status = PlatformAbstractionLayer::getInstance()->palNfcSendExtCmd(
      sizeof(getNtagNtfConfig), getNtagNtfConfig, &rsp_len, rsp);
  if ((status == NFCSTATUS_SUCCESS) &&
      (rsp[PROP_NTF_STATUS_INDEX] == NTAG_NTF_ENABLE_STATE)) {
    uint8_t setNtagNtfConfig[] = {0x20, 0x02, 0x05, 0x01,
                                  0xA1, 0xDA, 0x01, 0x00};
    status = PlatformAbstractionLayer::getInstance()->palNfcSendExtCmd(
        sizeof(setNtagNtfConfig), setNtagNtfConfig, &rsp_len, rsp);
    if (status != NFCSTATUS_SUCCESS) {
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "%s:Ntag Set Config : Failed",
                     __func__);
    }
  }
}
...
NFCSTATUS NxpNTag::waitForRfDiscRsp(NFCSTATUS status) {
  NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "NxpNTag::%s Enter", __func__);
  ...
  if (status == NFCSTATUS_FAILED &&
      NfcRfState::IDLE == RfStateMonitor::getInstance()->getNfcRfState()) {
    status = PlatformAbstractionLayer::getInstance()->palenQueueWrite(
        mNtagControl.mCurrentDiscCmd.data(),
...
NFCSTATUS NxpNTag::setNTagMode(uint16_t dataLen, uint8_t *pData) {
  ...
  case NTAG_ENABLE_PROP_OID:
    ...
    if (!mNtagControl.mNTagTimer.create(NULL, QPollTimerTimeoutCallback,
                                        &mNtagControl.ntagTimerId))
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "%s Timer create failed",
                     __func__);
  ...
  case NTAG_MODE_ENABLED_STATUS_OID: {
    ...
    PlatformAbstractionLayer::getInstance()->palSendNfcDataCallback(
        sizeof(NTAG_MODE_ENABLED_STATUS), NTAG_MODE_ENABLED_STATUS);
    return NFCSTATUS_EXTN_FEATURE_SUCCESS;
  }
  default:
    NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "NxpNTag::%s Invalid action",
                   __func__);
  ...
  PlatformAbstractionLayer::getInstance()->palSendNfcDataCallback(
      sizeof(NTAG_NCI_VENDOR_RSP), NTAG_NCI_VENDOR_RSP);
...
NFCSTATUS NxpNTag::processNTagSetSubState(NTagEvent event) {
  NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN,
                 "%s : Current sub state %d, Event: %d", __func__,
  ...
  case NTagSetSubState::NTAG_SET_SUB_STATE_IDLE: {
    ...
    if (status != NFCSTATUS_SUCCESS) {
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "%s : sendRfDeactivate failed",
                     __func__);
      notifyOnError = true;
      break;
    }
    mNTagSetSubState = NTagSetSubState::NTAG_SET_SUB_STATE_WAIT_FOR_RF_IDLE_RSP;
    status = waitForRfDiscRsp(status);
    if (status != NFCSTATUS_SUCCESS) {
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "%s : waitForRfDiscRsp failed",
                     __func__);
      notifyOnError = true;
      break;
    }
  ...
  case NTagSetSubState::NTAG_SET_SUB_STATE_WAIT_FOR_RF_IDLE_RSP: {
    if (event == NTagEvent::ACTION_NTAG_RF_DEACTIVATE_IDLE &&
      ...
      if (status != NFCSTATUS_SUCCESS) {
        NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                       "%s : sendNTagPropConfig(%s) failed", __func__,
                       enable ? "true" : "false");
        notifyOnError = true;
        break;
      }
      ...
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                     "%s : Unexpected event %d in WAIT_FOR_RF_IDLE_RSP",
                     __func__, event);
      notifyOnError = true;
    }
    break;
  ...
  case NTagSetSubState::NTAG_SET_SUB_STATE_WAIT_FOR_PROP_CMD_RSP: {
    if (event == NTagEvent::ACTION_NTAG_PROP_NTF_SET_STATUS &&
      ...
      if (status != NFCSTATUS_SUCCESS) {
        NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "%s : sendRfDiscCmd failed",
                       __func__);
        notifyOnError = true;
        break;
      }
      ...
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                     "%s : Unexpected event %d in WAIT_FOR_PROP_CMD_RSP",
                     __func__, event);
      notifyOnError = true;
    }
    break;
  ...
  case NTagSetSubState::NTAG_SET_SUB_STATE_WAIT_FOR_RF_DISC_RSP: {
    ...
    if (mWaitingforDiscRsp) {
      ...
      NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "%s : mNTagDiscRspCv signaled",
                     __func__);
    }
  ...
  default: {
    NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "%s : Invalid sub state %d",
                   __func__, mNTagSetSubState);
    notifyOnError = true;
    break;
  }
  }
  ...
  if (notifyOnError && mWaitingforDiscRsp) {
    ...
    mNTagDiscRspCv.signal();
    NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN,
                   "%s : mNTagDiscRspCv signaled due to error", __func__);
  }
...
void NxpNTag::updateState(NTagState state) {
  NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "%s : NTag state old:%d new:%d",
                 __func__, mNTagState, state);
  mNTagState = state;
}
...
NFCSTATUS NxpNTag::sendLpcdWoPoll(bool flag) {
  ...
  return PlatformAbstractionLayer::getInstance()->palenQueueWrite(
      &LPCD_WOUT_POLL_SET[0], LPCD_WOUT_POLL_SET.size());
}

NFCSTATUS NxpNTag::processNTagEvent(NTagEvent event) {
  NFCSTATUS status = NFCSTATUS_FAILED;
  NTagState mState = NTagState::NTAG_STATE_IDLE;
  NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "%s : Current state %d Event:%d",
                 __func__, mNTagState, event);
  ...
  case NTagState::NTAG_STATE_RF_DISCOVERY: {
    ...
    case NTagEvent::ACTION_NTAG_RF_DISCOVERY:
      ...
      if (mWaitingforDiscRsp) {
        mWaitingforDiscRsp = false;
        mNTagDiscRspCv.signal();
        NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "%s : mNTagDiscRspCv signal",
                       __func__);
      }
      break;
  ...
  default:
    status = NFCSTATUS_FAILED;
    NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "%s : Invalid state:%d", __func__,
                   mNTagState);
    break;
  }
  if (status == NFCSTATUS_SUCCESS && mNTagState != mState)
    updateState(mState);
  return status;
}
    ...
    if (!mNtagControl.isScreenOff) {
      ...
      if (!mNtagControl.mNtagUid.empty()) {
        ...
        if (PlatformAbstractionLayer::getInstance()->palGetNxpNumValue(
                NAME_NXP_NTAG_REMOVAL_TIMER_VALUE, &num, sizeof(num))) {
          if (num >= NXP_TIMER_MIN_SEC && num <= NXP_TIMER_MAX_SEC) {
            time_ms = num * 1000U;
          }
        }
        NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN,
                       "%s Tag Uid is found and starting the Timer", __func__);
        if (!mNtagControl.mNTagTimer.set((time_ms), NULL,
                                         QPollTimerTimeoutCallback,
                                         &mNtagControl.ntagTimerId)) {
          NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN,
                         "%s Failed to start load change Timer", __func__);
      ...
      if (status != NFCSTATUS_SUCCESS)
        NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN,
                       "NxpNTag::%s Failed to trigger RF discovery", __func__);
    } else {
      // Screen OFF state, after load change ntf LPCD is getting reset to
      // without poll off
      if (mNtagControl.mLpcdWoutPoll) {
        NFCSTATUS status = sendLpcdWoPoll(false);
        if (status != NFCSTATUS_SUCCESS) {
          NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN,
                         "%s: LPCD_WOUT_POLL_SET_OFF cmd failed", __func__);
          return NFCSTATUS_EXTN_FEATURE_FAILURE;
        } else {
          mNtagControl.mLpcdWoutPoll = false;
          ...
          if (status != NFCSTATUS_SUCCESS)
            NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                           "NxpNTag::%s Failed to trigger RF discovery",
                           __func__);
        }
      }
    }
...
void NxpNTag::handleNTagPresenceCheckRsp() {
  ...
  if (!(mNtagControl.mNtagDetectStatus & NTAG_PRESENCE_CHECK_TIMER_STATUS)) {
    ...
    if (!PlatformAbstractionLayer::getInstance()->palGetNxpNumValue(
            NAME_NXP_NTAG_PRESENCE_CHECK_TIMEOUT, &timeout, sizeof(timeout))) {
      NXPLOG_EXTNS_W(
          NXPLOG_ITEM_NXP_GEN_EXTN,
    ...
    if (!(mNtagControl.mNtagDetectStatus & NTAG_PRESENCE_CHECK_TIMEOUT)) {
      if (!QPollTimerStart(timeout)) {
        NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                       "NxpNTag::%s Failed to start timer", __func__);
      }
...
NFCSTATUS NxpNTag::handleNTagNciRsp(uint8_t *pData, uint16_t dataLen) {
  ...
  case NCI_MSG_RF_DEACTIVATE: {
    ...
    if (mNtagControl.mQPOLLMode == NFC_RF_DISC_RESTART &&
      ...
      PlatformAbstractionLayer::getInstance()->palSendNfcDataCallback(
          rfDeAct_Rsp.size(), &rfDeAct_Rsp[0]);
      return NFCSTATUS_EXTN_FEATURE_SUCCESS;
    }
    break;
  }
  ...
  case NCI_MSG_RF_ISO_DEP_NAK_PRESENCE:
    ...
    break;
  default:
    NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN, "%s: unexpected msg type received",
                   __func__);
    break;
  }
...
NFCSTATUS NxpNTag::handleNTagNciNtf(uint8_t *pData, uint16_t dataLen) {
  ...
  case NCI_MSG_RF_DEACTIVATE:
    if (mNtagControl.isScreenOff && mNtagControl.isCeStarted) {
      ...
      processNTagEvent(NTagEvent::ACTION_NTAG_RF_DEACTIVATE_IDLE);
      NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN,
                     "NxpNTag::%s Sending discovery in screen off", __func__);
      return NFCSTATUS_EXTN_FEATURE_FAILURE;
    }
    ...
    */
    if (mNtagControl.isRfNtfSent && dataLen == 5 &&
  ...
  if (mNtagControl.isNTagNtfEnabled) {
    if (dataLen == 7 && (pData[NCI_GID_INDEX] == NCI_MT_CMD) &&
      ...
      if (pData[6] == CON_DISC_POLL_DISABLE) {
        ...
        if (!mNtagControl.mLpcdWoutPoll &&
            (NfcRfState::IDLE ==
                 RfStateMonitor::getInstance()->getNfcRfState() ||
             NfcRfState::DISCOVER ==
                 RfStateMonitor::getInstance()->getNfcRfState())) {
          const NFCSTATUS status = sendLpcdWoPoll(true);
          if (status != NFCSTATUS_SUCCESS)
    ...
    if (!(dataLen < 5) && (pData[NCI_GID_INDEX] == 0x21) &&
      ...
      NFCSTATUS status =
          PlatformAbstractionLayer::getInstance()->palenQueueWrite(
              &mNtagControl.mCurrentDiscCmd[0],
              mNtagControl.mCurrentDiscCmd.size());
      if (status != NFCSTATUS_SUCCESS)
        return NFCSTATUS_EXTN_FEATURE_FAILURE;
      return NFCSTATUS_EXTN_FEATURE_SUCCESS;
...
NFCSTATUS NxpNTag::processRfDiscCmd(std::vector<uint8_t> &rfDiscCmd) {
  ...
  constexpr uint8_t RF_DISC_CMD_NO_OF_CONFIG_INDEX = 3;
  ...
  if (msgType == NCI_MSG_RF_DISCOVER) {
    ...
    if (!PlatformAbstractionLayer::getInstance()->palGetNxpNumValue(
            NAME_NXP_NTAG_DETECTION_TIMEOUT_VALUE, &timeout, sizeof(timeout))) {
      NXPLOG_EXTNS_W(NXPLOG_ITEM_NXP_GEN_EXTN,
  ...
  if (msgType == NCI_MSG_RF_DEACTIVATE) {
    ...
    if (!(mNtagControl.mNtagDetectStatus & NTAG_PRESENCE_CHECK_TIMER_STATUS) &&
      ...
      unsigned long timeout = NTAG_DETECT_TIMER_VALUE;
      if (!PlatformAbstractionLayer::getInstance()->palGetNxpNumValue(
              NAME_NXP_NTAG_DETECTION_TIMEOUT_VALUE, &timeout,
              sizeof(timeout))) {
        NXPLOG_EXTNS_W(
            NXPLOG_ITEM_NXP_GEN_EXTN,
            "%s: Failed to get NTAG detect timer , using default: %lu",
...
NFCSTATUS NxpNTag::sendNTagPropConfig(bool flag) {
  ...
  NFCSTATUS status;
  NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "NxpNTag::%s Flag: %d", __func__,
                 flag);
  if (!flag) {
  ...
  status = PlatformAbstractionLayer::getInstance()->palenQueueWrite(
      &setPropNtfEnable[0], setPropNtfEnable.size());
  if (status != NFCSTATUS_SUCCESS)
...
NFCSTATUS NxpNTag::sendRfDiscCmd(uint8_t pollMode) {
  ...
  if (NfcRfState::IDLE != RfStateMonitor::getInstance()->getNfcRfState()) {
    if (NFCSTATUS_SUCCESS != sendRfDeactivate())
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                     "%s Failed to send RF deactivate cmd", __func__);
...
void NxpNTag::processNTagDetectNtf(const std::vector<uint8_t> &uid) {
  ...
  PlatformAbstractionLayer::getInstance()->palSendNfcDataCallback(
      ntagDetectNtf.size(), ntagDetectNtf.data());
}
...
bool NxpNTag::isNTagReadComplete(const std::vector<uint8_t> &uid) {
  ...
  if (mNtagControl.isScreenOff) {
    if (!isSameUid) {
      ...
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                     "NxpNTag::%s Sent UID in Screen Off", __func__);
    }
  ...
  if (isSameUid && (mNtagControl.mNtagDetectStatus &
    ...
    const NFCSTATUS status =
        processNTagEvent(NTagEvent::ACTION_NTAG_UID_MATCHED);
    if (status != NFCSTATUS_SUCCESS)
      NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                     "NxpNTag::%s Failed to trigger RF discovery", __func__);
...
NFCSTATUS NxpNTag::handleRfIntfActivated(uint8_t *pData, uint16_t dataLen) {
  ...
  if (dataLen <= (NTAG_UID_START_INDEX + uidLength)) {
    NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                   "NxpNTag::%s Invalid RF Interface Notification", __func__);
    mNtagControl.mNtagDetectStatus = 0x00;
    return NFCSTATUS_EXTN_FEATURE_FAILURE;
  }

  // Extract UID directly from pData
  const std::vector<uint8_t> extractedUid(
      pData + NTAG_UID_START_INDEX, pData + NTAG_UID_START_INDEX + uidLength);
...
void NxpNTag::updateNTagRemoveStatus() {
  ...
  // Notify removal and reset NTAG state
  PlatformAbstractionLayer::getInstance()->palSendNfcDataCallback(
      sizeof(NTAG_MODE_REMOVED_STATUS), NTAG_MODE_REMOVED_STATUS);
  mNtagControl.mNtagUid.clear();
  mNtagControl.mNtagDetectStatus = NTAG_REMOVAL_STATUS;
}
...
void NxpNTag::checkNTagRemoveStatus() {
  ...
  if (!mNtagControl.mNtagUid.empty() &&
      (mNtagControl.mNtagDetectStatus & NTAG_REMOVAL_STATUS) !=
          NTAG_REMOVAL_STATUS) {
    NXPLOG_EXTNS_E(NXPLOG_ITEM_NXP_GEN_EXTN,
                   "NxpNTag::%s: NTAG removed and clearing the status",
                   __func__);
    updateNTagRemoveStatus();
  }
  // In screen off Ntag not detected confirmed with timeout, hence restoring
  // LPCD without poll
...
void NxpNTag::QPollTimerTimeoutCallback(union sigval val) {
  NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "NxpNTag::%s Timer expired...",
                 __func__);
  NxpNTag::getInstance()->checkNTagRemoveStatus();
}

bool NxpNTag::QPollTimerStart(unsigned long sec) {
  NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "%s Starting %ld milliseconds timer",
                 __func__, sec * 1000);
  if (!mNtagControl.mNTagTimer.set((sec * 1000), NULL,
                                   QPollTimerTimeoutCallback,
                                   &mNtagControl.ntagTimerId)) {
    NXPLOG_EXTNS_E("%s Timer start failed", __func__);
    return false;
  }
  return true;
}
...
void NxpNTag::QPollTimerStop() {
  NXPLOG_EXTNS_D(NXPLOG_ITEM_NXP_GEN_EXTN, "NxpNTag::%s  Stopping timer...",
                 __func__);
  mNtagControl.mNTagTimer.kill(&mNtagControl.ntagTimerId);
}
