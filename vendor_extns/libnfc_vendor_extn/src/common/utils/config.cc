...
bool parseBytesString(std::string in, std::vector<uint8_t>& out) {
  const vector<string> values = Split(in, ":");
...
bool ConfigFile::updateConfig(const std::string& key, ConfigValue& value) {
  const std::map<std::string, ConfigValue>::iterator it = values_.find(key);
  if (it != values_.end()) {
    it->second = value;
    return true;
  }
...
void ConfigFile::parseFromFile(const std::string& file_name) {
  string config;
  const bool config_read = ReadFileToString(file_name, &config);
  CHECK(config_read);
...
void ConfigFile::parseFromString(const std::string& config) {
  ...
  while (std::getline(ss, line)) {
    ...
    const string value_string(Trim(line.substr(search + 1, string::npos)));

    ConfigValue value;
    const bool value_parsed = value.parseFromString(value_string);
    CHECK(value_parsed);
    if (updateNciCfg) {
