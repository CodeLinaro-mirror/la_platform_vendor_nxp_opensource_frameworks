...
public:
  ...
  /**
   * @brief updates the write completion callback status
  ...
  static inline void finalize() {
    if (sNfcExtensionWriter != nullptr) {
      sNfcExtensionWriter.reset();
    }
  }

  // Setter method for lastNciCmd
  void setLastNciCmd(const std::vector<uint8_t>& cmd) {
    lastNciCmd = cmd;
  }

  // Getter method for lastNciCmd
  const std::vector<uint8_t>& getLastNciCmd() const {
    return lastNciCmd;
  }
...
private:
  ...
  static std::unique_ptr<NfcExtensionWriter> sNfcExtensionWriter;
  NfcExtensionWriter();
  ~NfcExtensionWriter();
  ...
  friend struct std::default_delete<NfcExtensionWriter>;
};
