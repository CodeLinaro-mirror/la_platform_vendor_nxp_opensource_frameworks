...
public:
   ...
   *
   */
  void updateFwDnldStatus(uint8_t status);
  /**
  ...
  static inline void finalize() {
    if (sProprietaryExtn != nullptr) {
      sProprietaryExtn.reset();
    }
  }

private:
  static std::unique_ptr<ProprietaryExtn> sProprietaryExtn;
  /**
  ...
  friend struct std::default_delete<ProprietaryExtn>;
};
