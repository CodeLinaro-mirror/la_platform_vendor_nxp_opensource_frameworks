...
public interface INxpNfcAdapter {
     ...
     * @return whether the update of state is MPOS_STATUS_SUCCESS, MPOS_STATUS_FAILED,
     ...
     *
     * @param pkg package name of the caller
     * @return TRUE if reader mode is started FALSE if reader mode is not started
     * @throws IOException If a failure occurred during reader mode set or reset
     */
     ...
     * <li>This api shall be called only NfcService is enabled.
     *
     * @return status :-0x00 :SUCCESS 0x01 - 0x06: NCI Status Codes : Refer NCI spec v2.3 Table 140
     *     0x07 : NFC off 0x0B : Disabled 0x0C : Config not defined 0x0D : Feature not supported by
     *     platform 0x0E : EACSTATUS_ERROR_MESSAGE_CORRUPTED byte[0] indicates status, byte[1]
     *     onwards AID in TLV format. Number of Aid's : 1 Byte Protocol type. : 1 Byte Length of AID
     *     : 1 Byte Aid Entry : AID (x Bytes supported 5 <= x <= 16)
     ...
     * @return status :-0x00 :SUCCESS 0x01 - 0x06: NCI Status Codes : Refer NCI spec v2.3 Table 140
     *     0x07 : NFC off 0x0B : Disabled 0x0C : Config not defined 0x0D : Feature not supported by
     *     platform 0x0E : EACSTATUS_ERROR_MESSAGE_CORRUPTED
     *     <p>Requires {@link android.Manifest.permission#NFC} permission.
     */
     ...
     * @return status :-0x00 :SUCCESS 0x01 - 0x06: NCI Status Codes : Refer NCI spec v2.3 Table 140
     *     0x07 : NFC off 0x0B : Disabled 0x0C : Config not defined 0x0D : Feature not supported by
     *     platform 0x0E : EACSTATUS_ERROR_MESSAGE_CORRUPTED byte[0] array : RF params <aid-0 index>
     *     <19 bytes data> ... : <aid-n index> <19 bytes data>
     *     <p>Requires {@link android.Manifest.permission#NFC} permission.
     */
     ...
     *   <li>This api shall be called only when there are no NFC transactions ongoing.
     *   <li>Limit the NFC controller to reader mode while this Activity is in the foreground.
     * </ul>
     *
     ...
     * @param mode to ENABLE_QTAG_ONLY_MODE. APPEND_QTAG_MODE with input pollTech. DISABLE_QTAG_MODE
     *     & reset to default discovery.
     * @param pollTech to append QPoll in reader mode.
     * @param delay_value PRESENCE_CHECK_DELAY
     * @return whether the update of state is QTag_STATUS_SUCCESS, QTag_STATUS_FAILED,
     ...
     *     & reset to default discovery.
     * @param pollTech to append QPoll in reader mode.
     * @param Bundle options - Please use this bundle set delay_value and Readermode annotations
     * @return whether the update of state is QTag_STATUS_SUCCESS, QTag_STATUS_FAILED,
     * @throws IOException If a failure occurred during QTag RF mode set or reset
     */
    public int enableQTag(
    ...
    /**
     * This api is called by applications to update the NFC configurations which are already part of
     * libnfc-nci.conf
     *
     * <p>Requires {@link android.Manifest.permission#NFC} permission.
     *
     ...
     * @apiNote String should have KEY and assigned with Value in Hexadecimal. Refer below sample.
     *     POLLING_TECH_MASK=0x4F
     * @return whether the update of configuration is success or not. 0xFF - failure 0x00 - success
     ...
     *
     * @return byte array of fwVersion fwVersion byte array of length 3 - Success byte[0] - Major
     *     version byte[1] - Minor version byte[2] - Rom version fwVersion byte array with all bytes
     *     0x00 - Failure
     */
     ...
     *
     * @param callbacks : callback object to be register. /** This API registers the callback to get
     *     SRD Timeout Events.
     * @param callbacks : callback object to be register.
     */
    ...
    /**
     * This API unregisters the Application callbacks to be called for SRD Timeout notifications.
     */
    ...
    /**
     * This is the API to be called to check Dual Antenna feature is supported or not.
     * <li>This api shall be called only when NfcService is enabled.
     * <li>This api shall be called only when there are no NFC transactions ongoing.
     * </ul>
     *
     ...
     * @return whether the update of state is ENABLE_DISABLE_STATUS_SUCCESS,
    ...
    /**
     * This is the API to be called to get the discovery technology on both antennas
     * <li>This api shall be called only when NfcService is enabled.
     * <li>This api shall be called only when there are no NFC transactions ongoing.
     * </ul>
     *
     ...
     * </ul>
     *
     * @return TRUE if NTag mode is enabled FALSE if NTag mode is disabled
     ...
     * </ul>
     *
     * @param activity activity the Activity that requests the adapter to be in reader mode.
     * @param mNTagStatusCallback the callback to be called when NTAG is added/removed UID value and
     *     detected status will be to updated when tag is added. Removed status will be update on
     *     tag removal.
     * @param mode to ENABLE_NTAG. DISABLE_NTAG & reset to default discovery.
     * @return whether the update of state is 0x00 - STATUS_SUCCESS, 0x01 - STATUS_FAILED,
     * @throws IOException If a failure occurred during NTag RF mode set or reset
     */
