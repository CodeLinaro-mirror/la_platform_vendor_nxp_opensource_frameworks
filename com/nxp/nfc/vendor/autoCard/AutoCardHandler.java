...
public class AutoCardHandler implements INxpNfcNtfHandler {
    ...
    private static final int NFC_NCI_PROP_GID = 0x2F;
    private static final int NXP_NFC_AUTO_CARD_SUB_GID = 0x50;
    private static final int NXP_NFC_AUTO_CARD_SUB_OID_INDEX = 0;
    private static final int AUTO_CARD_AID_PAYLOAD_OFFSET = 0x03;
    private static final int AUTO_CARD_PAYLOAD_OFFSET = 0x02;
    private static final int AUTO_CARD_STATUS_RSP_INDEX = 0x03;
    private static final int AUTO_CARD_CONFIG_INDEX = 0x01;
    private static final int AUTO_CARD_ENABLE_INDEX = 0x02;
    private static final int AID_LENGTH_INDEX = 0x02;
    private static final int STR_AID_MIN_LENGTH = 4;
    private static final int STR_AID_MAX_LENGTH = 16;

    public static final Object sAutoCardSync = new Object();
    ...
    public void onVendorNciNotification(int gid, int oid, byte[] payload) {
        ...
        NxpNfcLogger.d(
                TAG, "Sub-GidOid: " + subGidOid + ", Notification Type: " + notificationType);

        synchronized (sAutoCardSync) {
            if (subGidOid == NXP_NFC_AUTO_CARD_SUB_GID
                ...
                if (payload.length > 5) {
                    ...
                    NxpNfcLogger.d(
                            TAG, "Applet Status Ntf:" + NxpNfcUtils.toHexString(appletStatus));
                }
            }
        }
    }
    ...
    public @AutoCardStatus int enableAutoCard(AutoCardStatusCallback mAutoCardStatusCallback)
        ...
        if (mAutoCardStatusCallback == null) return NfcAdapter.SEND_VENDOR_NCI_STATUS_FAILED;
        ...
        mNxpNciPacketHandler.registerNtfCallback(Executors.newSingleThreadExecutor(), this);

        this.mAutoCardStatusCallback = mAutoCardStatusCallback;

        try {
            byte[] vendorRsp =
                    mNxpNciPacketHandler.sendVendorNciMessage(
                            NxpNfcConstants.NFC_NCI_PROP_GID,
                            NxpNfcConstants.NXP_NFC_PROP_OID,
                            payload);
            if (vendorRsp == null) {
    ...
    public byte[] getAutoCardAID() throws IOException {
        ...
        try {
            ...
            if (vendorRsp.length > AUTO_CARD_STATUS_RSP_INDEX) {
                if (vendorRsp[AUTO_CARD_STATUS_RSP_INDEX] != INxpNfcAdapter.STATUS_SUCCESS) {
                    byte[] rspAid = new byte[1];
                    rspAid[0] = vendorRsp[AUTO_CARD_STATUS_RSP_INDEX];
                    NxpNfcLogger.e(TAG, "Get Autocard failed Rsp!!" + rspAid);
                    return rspAid;
                } else {
                    byte[] rspAid = new byte[vendorRsp.length - AUTO_CARD_AID_PAYLOAD_OFFSET];
                    System.arraycopy(
                            vendorRsp,
                            AUTO_CARD_AID_PAYLOAD_OFFSET,
                            rspAid,
                            0,
                ...
                byte[] rspAid = new byte[1];
                rspAid[0] = (byte) NfcAdapter.SEND_VENDOR_NCI_STATUS_FAILED;
                NxpNfcLogger.e(TAG, "Get Autocard Failed!!");
    ...
    public @AutoCardStatus int setAutoCardAID(byte[] aids) throws IOException {
        if (aids == null) return INxpNfcAdapter.EACSTATUS_ERROR_INVALID_PARAM;
    ...
    public byte[] getAutoCardRfParams() throws IOException {
        ...
        try {
            ...
            if (vendorRsp.length > AUTO_CARD_AID_PAYLOAD_OFFSET) {
                ...
                byte[] rspAid = new byte[1];
                rspAid[0] = (byte) NfcAdapter.SEND_VENDOR_NCI_STATUS_FAILED;
                NxpNfcLogger.e(TAG, "Get Autocard RfParams Failed!!");
                NxpNfcLogger.e(TAG, "sendVendorNciCmd  response:" + vendorRsp.length);
                return rspAid;
            }
        } catch (Exception e) {
    ...
    public @AutoCardStatus int setAutoCardRfParams(byte[] aidRfParams) throws IOException {
        if (aidRfParams == null) return INxpNfcAdapter.EACSTATUS_ERROR_INVALID_PARAM;
        ...
        try {
            System.arraycopy(aidRfParams, 0, payload, AUTO_CARD_PAYLOAD_OFFSET, aidRfParams.length);
    ...
    public @AutoCardStatus int setAutoCardAppletStatus(byte[] appletStatus) throws IOException {
        ...
        payload[NXP_NFC_AUTO_CARD_SUB_OID_INDEX] = (byte) NXP_NFC_AUTO_CARD_SUB_GID;
        payload[AUTO_CARD_CONFIG_INDEX] = (byte) AutoCardSubOid.SetAppletStatus.value;

        try {
            System.arraycopy(
                    appletStatus, 0, payload, AUTO_CARD_PAYLOAD_OFFSET, appletStatus.length);
    ...
    public @AutoCardStatus int setStrReaderProfiles(byte[] strProfileData) throws IOException {
        if (strProfileData == null) return INxpNfcAdapter.EACSTATUS_ERROR_INVALID_PARAM;
        if (mNfcAdapter.getAdapterState() == NfcAdapter.STATE_OFF) {
            NxpNfcLogger.e(TAG, "NFC is disabled");
            return INxpNfcAdapter.EACSTATUS_ERROR_NFC_IS_OFF;
        }
        byte[] payload = new byte[strProfileData.length + AUTO_CARD_PAYLOAD_OFFSET];
        payload[NXP_NFC_AUTO_CARD_SUB_OID_INDEX] = (byte) NXP_NFC_AUTO_CARD_SUB_GID;
        payload[AUTO_CARD_CONFIG_INDEX] = (byte) AutoCardSubOid.SetStrProfile.value;
        try {
            System.arraycopy(
                    strProfileData, 0, payload, AUTO_CARD_PAYLOAD_OFFSET, strProfileData.length);
            byte[] vendorRsp =
                    mNxpNciPacketHandler.sendVendorNciMessage(
                            NxpNfcConstants.NFC_NCI_PROP_GID,
                            NxpNfcConstants.NXP_NFC_PROP_OID,
                            payload);
            if (vendorRsp == null) {
                NxpNfcLogger.e(TAG, "setStrReaderProfiles response: Null");
                return NfcAdapter.SEND_VENDOR_NCI_STATUS_FAILED;
            }
            if (vendorRsp.length > AUTO_CARD_STATUS_RSP_INDEX) {
                return vendorRsp[AUTO_CARD_STATUS_RSP_INDEX];
    ...
    public @AutoCardStatus int setStrActivatedAID(byte[] aid) throws IOException {
        if (aid == null || aid.length > STR_AID_MAX_LENGTH || aid.length < STR_AID_MIN_LENGTH) {
            return INxpNfcAdapter.EACSTATUS_ERROR_INVALID_PARAM;
        }
        if (mNfcAdapter.getAdapterState() == NfcAdapter.STATE_OFF) {
            NxpNfcLogger.e(TAG, "NFC is disabled");
        ...
        byte[] payload = new byte[aid.length + AUTO_CARD_PAYLOAD_OFFSET + 1];
        payload[NXP_NFC_AUTO_CARD_SUB_OID_INDEX] = (byte) NXP_NFC_AUTO_CARD_SUB_GID;
        payload[AUTO_CARD_CONFIG_INDEX] = (byte) AutoCardSubOid.SetStrActivateAid.value;
        payload[AID_LENGTH_INDEX] = (byte) aid.length;
        try {
            System.arraycopy(aid, 0, payload, (AUTO_CARD_PAYLOAD_OFFSET + 1), aid.length);
            byte[] vendorRsp =
                    mNxpNciPacketHandler.sendVendorNciMessage(
                            NxpNfcConstants.NFC_NCI_PROP_GID,
                            NxpNfcConstants.NXP_NFC_PROP_OID,
